...
def mitigation_planner(state: DisruptionState) -> DisruptionState:
    ...
    brief = {
        "plan_id": plan_id,
        "affected_orders_count": len(state.get("impacted_orders", [])),
        "required_capacity": req_cap,
        ...
        "summary": (f"Disrupted supplier {state['affected_supplier_id']} cannot fulfil "
                    f"{len(state.get('impacted_orders', []))} orders "
                    f"({req_cap} units). Recommend {winner['name']} "
